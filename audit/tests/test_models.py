...
class AuditEventTestCase(TestCase):
    def test_target_repr_survives_target_deletion(self):
        ...
        event = AuditEvent.objects.create(subject=actor, action='updated', target=target,
    ...
    def test_str_includes_subject_action_and_target(self):
        # Arrange
        actor = User.objects.create(username='staff')

        # Act
        event = AuditEvent.objects.create(subject=actor, action='created', target_repr='thing')
