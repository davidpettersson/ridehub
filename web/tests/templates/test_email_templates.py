...
class TestVerificationReminderEmail(BaseEmailTestCase):
    def setUp(self):
        super().setUp()
        self.user = User.objects.create_user(
    ...
    def test_links_are_absolute(self):
        # Act
        html_content = render_to_string('email/verification_reminder.html', self.context)
    ...
    def test_text_version_has_absolute_urls(self):
        # Act
        text_content = render_to_string('email/verification_reminder.txt', self.context)
    ...
    def test_contains_the_verification_url(self):
        # Act
        html_content = render_to_string('email/verification_reminder.html', self.context)
        text_content = render_to_string('email/verification_reminder.txt', self.context)

        # Assert
        self.assertIn(self.context['verification_url'], html_content)
        self.assertIn(self.context['verification_url'], text_content)
